// Go implementation of the Steinhaus–Johnson–Trotter algorithm
// Adapted from http://rosettacode.org/wiki/Permutations_by_swapping#Go
...
func PermuteIter(p []string) func() int {
	f := pf(len(p))
	return func() int {
		return f(p)
	}
...
func pf(n int) func([]string) int {
	sign := 1
	switch n {
	case 0, 1:
		return func([]string) (s int) {
			s = sign
			sign = 0
			return
		}
